...
@st.cache_data
def filter_data(category, df):
    if category == "All":
        return df
    return df[df['Category'] == category]

df_popin = filter_data(event_category, df_popin)
...
if "latitude" not in df_popin.columns or "longitude" not in df_popin.columns:
    ...
    df_popin = get_geocoded_data(df_popin)
...
if st.session_state.graph_selection == "Event Performance Overview":
    ...
    with col3:
        ...
        if not df_popin.empty:
            ...
            @st.cache_data
            def compute_avg_attendance(df):
                return df.groupby('Category')['Attendees'].mean().reset_index()

            average_attendance = compute_avg_attendance(df_popin)
...
# ----------------- TITLE DISTRIBUTION ----------------- #
    ...
    wordcloud = generate_wordcloud(' '.join(df_popin['Event_Name'].dropna()))
